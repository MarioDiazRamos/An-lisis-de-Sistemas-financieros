...
Versión corregida para manejar adecuadamente warnings sobre tipos no booleanos
"""
...
class MineroReglas:
    ...
    def extraer_reglas(self, df):
        ...
        try:
            ...
            self.logger.error(f"Error al extraer reglas de asociación: {str(e)}", exc_info=True)
            return pd.DataFrame()
    ...
    def _preparar_datos(self, df, col_categoricas):
        ...
        for col in col_categoricas:
            ...
            for val in valores:
                ...
                # Convertir explícitamente a tipo bool para evitar DeprecationWarning
                # La advertencia ocurre cuando se usan valores enteros (0, 1) en lugar de booleanos
                df_preparado[nombre_col] = (df[col] == val).astype(bool)
    ...
    def guardar_reglas(self, reglas, ruta_archivo):
        ...
        """
        # Verificar si hay reglas para guardar
        if reglas.empty:
            self.logger.warning(f"No hay reglas para guardar en {ruta_archivo}")
            return
        ...
        try:
            ...
            self.logger.error(f"Error al guardar reglas en {ruta_archivo}: {str(e)}")
            raise
    ...
    def evaluar_regla_en_datos(self, regla, df_discretizado):
        ...
        # Combinar condiciones con AND
        if condiciones_cons:  # Proteger contra lista vacía
